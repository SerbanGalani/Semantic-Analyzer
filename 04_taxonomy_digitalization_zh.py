...
TAXONOMY_NAME    = "Digitalization_Relational_v2_2_ZH"
TAXONOMY_VERSION = "2.2.1-zh"
...
class DigitalizationRelationalTaxonomy(TaxonomyProvider):
    ...
    def __init__(self):
        ...
        def _compile_term_set(values: Set[str]) -> List[re.Pattern]:
        ...
        self._external_actor_res = _compile_term_set(EXTERNAL_ACTORS)
        self._relational_verb_res = _compile_term_set(RELATIONAL_VERBS)
        self._digital_marker_res = _compile_term_set(DIGITAL_MARKERS)
...
# ============================================================================
# ZH DELTA APPENDIX — bilingual EN + ZH support for Chinese texts
# Added as a non-destructive appendix so the original taxonomy stays readable.
# ============================================================================

EXTERNAL_ACTORS.update({
    "供应商", "合作伙伴", "伙伴", "经销商", "分销商", "客户", "生态", "生态系统",
    "第三方", "网络", "互补者", "渠道伙伴", "交易伙伴", "物流伙伴", "服务伙伴",
    "转售商", "开发者", "平台参与方", "上下游", "产业链伙伴",
    "client", "clients", "merchant", "merchants", "operator", "operators",
    "carrier", "oem", "retailer", "retailers", "marketplace seller",
    "installers", "franchisee",
})

RELATIONAL_VERBS.update({
    "集成", "整合", "连接", "对接", "共享", "交换", "协同", "协作", "同步",
    "接入", "接入到", "赋能", "联通", "打通", "开放给", "开放", "链接", "绑定",
    "交易", "协作开发", "共创", "上线", "接入平台",
    "enable", "enabled", "federate", "federated", "interface", "interfaced",
    "link", "linked", "standardize", "standardized", "provide access",
    "expose", "orchestrate",
})

DIGITAL_MARKERS.update({
    "数据", "平台", "接口", "API", "门户", "分析", "系统", "数字化", "数字",
    "互操作", "控制塔", "共享", "交换", "集成", "生态", "合作伙伴", "供应商",
    "网络", "云", "云端", "物联网", "区块链", "ERP", "CRM", "工作流", "仪表板",
    "中间件", "电子数据交换", "EDI", "自动化", "数字孪生",
})

FALSE_POSITIVE_PATTERNS.update({
    "passport_travel_zh": [
        r"护照(?!.*(?:产品护照|数字产品护照|电池护照|碳足迹|可追溯|供应链))",
    ],
    "governance_board_only_zh": [
        r"(?:公司治理|董事会治理)(?!.*(?:平台|数字|数据|生态|接口|系统|供应商|合作伙伴))",
    ],
    "learning_internal_ld_zh": [
        r"(?:培训|学习|知识共享).*(?:员工|内部|人力资源|人才|学院)(?!.*(?:合作伙伴|供应商|生态|跨公司|平台|数据))",
    ],
    "transparency_investor_relations_zh": [
        r"(?:透明度|信息披露).*(?:投资者|股东|资本市场)(?!.*(?:平台|数据|数字|生态|接口|供应商|合作伙伴))",
    ],
    "customer_experience_consumer_only_zh": [
        r"(?:客户体验|消费者体验)(?!.*(?:B2B|工业客户|合作伙伴|经销商|分销商|服务伙伴))",
    ],
})

_ZH_KEYWORDS: Dict[str, List[str]] = {
    "D1_Partner_Onboarding_Access": ["合作伙伴入驻", "供应商入驻", "供应商注册", "经销商入驻", "伙伴门户", "自助注册"],
    "D2_Transactional_Integration": ["电子数据交换", "电子发票", "采购到付款", "订单到收款", "订单集成", "跨企业交易集成"],
    "D3_Collaborative_Planning_Visibility": ["协同计划", "协同预测", "供应链可视化", "控制塔", "共享需求计划", "实时库存可视化"],
    "D4_Shared_Data_Interoperability": ["数据共享平台", "共享数据空间", "主数据同步", "跨企业数据共享", "互操作平台", "联邦数据共享", "主权数据交换"],
    "D5_Channel_Partner_Interface": ["经销商门户", "分销商门户", "渠道伙伴门户", "伙伴关系管理", "渠道管理平台", "经销商管理系统"],
    "D6_CoInnovation_Digital_Servitization": ["数字服务化", "远程监控服务", "预测性维护", "结果导向合同", "工业客户数据共享", "联合创新"],
    "D7_Ecosystem_Orchestration_Complementors": ["生态系统编排", "开发者生态", "互补者", "API市场", "多边平台", "平台生态"],
    "D8_Traceability_Compliance_Network": ["端到端追溯", "数字产品护照", "供应商合规门户", "来源追踪", "区块链追溯", "电池护照", "产品碳足迹"],

    "T1_ERP_SCM_Backbone": ["ERP对接供应商", "供应链骨干系统", "采购系统集成", "SAP Ariba", "ERP供应链集成", "数字核心"],
    "T2_API_EDI_iPaaS_Middleware": ["API集成", "EDI", "电子数据交换", "中间件", "iPaaS", "B2B网关", "集成平台"],
    "T3_B2B_Platforms_Marketplaces": ["B2B平台", "采购平台", "供应商网络平台", "数字贸易平台", "市场平台", "伙伴平台"],
    "T4_Cloud_Data_Platforms": ["云平台", "数据平台", "行业云", "共享云平台", "伙伴数据湖", "云端数据共享"],
    "T5_IoT_Digital_Twins_Connected_Assets": ["工业物联网", "数字孪生", "联网资产", "远程监控", "传感器", "状态监测"],
    "T6_Identity_Access_Security": ["联邦身份", "身份与访问管理", "零信任", "安全数据交换", "单点登录", "访问控制"],
    "T7_Blockchain_DLT_Traceability": ["区块链追溯", "分布式账本", "智能合约", "数字贸易账本", "链上溯源", "许可链"],
    "T8_Workflow_Automation_LowCode": ["工作流自动化", "低代码", "无代码", "伙伴流程自动化", "审批流", "Power Platform"],

    "G1_Transparency_Visibility_Control": ["透明度", "可视化", "共享KPI", "伙伴监控", "供应商透明度", "行为监控", "控制塔治理"],
    "G2_Trust_Security_Assurance": ["可信数据共享", "审计追踪", "信任机制", "保障机制", "防止机会主义", "权力不对称", "相互依赖治理"],
    "G3_Data_Sovereignty_Ownership_Rights": ["数据主权", "数据所有权", "访问权", "使用控制", "同意式数据共享", "数据权利", "跨境数据治理"],
    "G4_Standards_Interoperability_Rules": ["数据标准", "互操作框架", "共享本体", "标准接口", "参考架构", "数据空间标准"],
    "G5_Orchestration_Roles_Incentives": ["生态治理", "参与规则", "收益分配", "平台治理", "角色分配", "激励机制", "平台采纳阻力"],
    "G6_Joint_Learning_Knowledge_Sharing": ["知识共享", "联合学习", "伙伴赋能", "协同分析", "共同开发", "跨企业学习"],
}

_ZH_ANCHORS: Dict[str, List[str]] = {
    "D1_Partner_Onboarding_Access": ["我们上线了合作伙伴入驻门户，将供应商和经销商注册、身份核验与系统开通流程全面数字化。"],
    "D2_Transactional_Integration": ["我们通过API和EDI将订单、发票和采购流程与供应商系统打通，实现跨企业交易自动化。"],
    "D3_Collaborative_Planning_Visibility": ["共享控制塔为供应商和物流伙伴提供实时需求、库存和补货信号。"],
    "D4_Shared_Data_Interoperability": ["公司建设了共享数据空间和统一数据模型，用于与生态伙伴开展跨企业数据交换。"],
    "D5_Channel_Partner_Interface": ["经销商和分销商通过统一伙伴门户处理订单、价格、服务工单和返利流程。"],
    "D6_CoInnovation_Digital_Servitization": ["我们与工业客户基于联网设备和运营数据共同提供远程监控与预测性维护服务。"],
    "D7_Ecosystem_Orchestration_Complementors": ["公司通过开放API和伙伴平台编排开发者与互补者生态，扩大平台参与度。"],
    "D8_Traceability_Compliance_Network": ["数字产品护照和区块链追溯平台连接上下游伙伴，实现端到端合规与来源管理。"],

    "T1_ERP_SCM_Backbone": ["ERP和供应链骨干系统已延伸到企业边界之外，用于对接供应商和渠道伙伴流程。"],
    "T2_API_EDI_iPaaS_Middleware": ["我们使用API、EDI和中间件连接伙伴系统，实现实时数据交换。"],
    "T3_B2B_Platforms_Marketplaces": ["B2B平台和采购市场成为供应商、客户与互补者协同的共同界面。"],
    "T4_Cloud_Data_Platforms": ["共享云数据平台将内部数据与伙伴数据连接起来，用于联合运营和分析。"],
    "T5_IoT_Digital_Twins_Connected_Assets": ["工业物联网和数字孪生使供应商、服务伙伴与客户能够共享设备运行信息。"],
    "T6_Identity_Access_Security": ["联邦身份和零信任访问控制支撑合作伙伴安全接入共享平台。"],
    "T7_Blockchain_DLT_Traceability": ["分布式账本和智能合约被用于多方溯源、贸易文件和供应商协同。"],
    "T8_Workflow_Automation_LowCode": ["低代码工作流工具自动化了涉及供应商、经销商和伙伴的跨企业审批流程。"],

    "G1_Transparency_Visibility_Control": ["共享仪表板和实时监控提高了伙伴网络的透明度，并降低了协调摩擦。"],
    "G2_Trust_Security_Assurance": ["可验证记录、审计追踪与安全协作机制提升了生态参与者之间的信任。"],
    "G3_Data_Sovereignty_Ownership_Rights": ["合作伙伴在平台上按照明确的数据所有权、访问权和使用权规则共享数据。"],
    "G4_Standards_Interoperability_Rules": ["共同的数据标准、参考架构和互操作规则支撑了生态协同。"],
    "G5_Orchestration_Roles_Incentives": ["平台所有者通过角色设定、激励机制和收益分配规则治理生态参与。"],
    "G6_Joint_Learning_Knowledge_Sharing": ["共享知识平台和联合学习机制帮助合作伙伴共同提升数字能力。"],
}


def _apply_zh_delta() -> None:
    all_cats: Dict[str, CategoryInfo] = {}
    all_cats.update(_D_APPLICATIONS)
    all_cats.update(_T_TECHNOLOGIES)
    all_cats.update(_G_GOVERNANCE)

    for code, terms in _ZH_KEYWORDS.items():
        cat = all_cats.get(code)
        if not cat:
            continue
        for term in terms:
            if term not in cat.keywords:
                cat.keywords.append(term)
            if term not in cat.keyword_tiers:
                cat.keyword_tiers[term] = 1

    for code, phrases in _ZH_ANCHORS.items():
        bucket = ANCHOR_PHRASES.setdefault(code, [])
        for phrase in phrases:
            if phrase not in bucket:
                bucket.append(phrase)


def _get_anchor_phrases_bilingual(self) -> Dict[str, List[str]]:
    return ANCHOR_PHRASES


_apply_zh_delta()
DigitalizationRelationalTaxonomy.get_anchor_phrases = _get_anchor_phrases_bilingual

# Rebuild singleton + compiled cache so the appended ZH delta is active.
TAXONOMY = DigitalizationRelationalTaxonomy()
PATTERN_CACHE = CompiledPatternCache()
PATTERN_CACHE.compile_from_provider(TAXONOMY)
